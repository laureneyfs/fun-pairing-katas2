...
describe("sumDigits", () => {
  test("returns input number when passed a single digit number", () => {
    const actual = sumDigits(5);
    const expected = 5;
    expect(actual).toBe(expected);
  });
  test("support for summing multi-digit inputs", () => {
    const actual = sumDigits(18);
    const expected = 9;
    expect(actual).toBe(expected);
  });
